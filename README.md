...
每次同步后，程序将邮件发件邮箱与飞书Base各字段中出现的邮箱做不区分大小写的精确匹配。邮箱索引只以不可逆SHA-256哈希保存在PostgreSQL，重启后直接复用，并每30分钟后台刷新。升级前同步的旧邮件也会自动补分类和重新匹配；未匹配邮件会在后续同步时重试。匹配成功后会安全写回“最近发件邮箱、最后联系时间、邮件同步状态、邮件分类”，并只把早期合作阶段推进为“已回复”。业务含义分类（感兴趣、报价、拒绝等）留到第四阶段接入AI后处理。
