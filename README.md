# Creator BD Agent — Phase 5.0.0

这是Creator BD Agent第五阶段的“人工确认SMTP回复工作台”。它保留IMAP只读同步、飞书匹配和AI分析，并新增：

- 为一个试点邮箱加密配置SMTP，单独测试后才能启用；
- 发送前在管理页显示固定发件邮箱、原始发件人和主题，并再次弹窗确认；
- 只有受`ADMIN_TOKEN`保护的人工确认接口能够发送，定时任务没有发送代码路径；
- 回复保留`In-Reply-To`、`References`和原主题线程；
- 使用数据库原子状态阻止重复点击和同一封回复重复发送；
- 记录不含正文和明文收件人的安全发送日志；
- 可选通过IMAP把已发送副本保存到指定Sent文件夹；Sent保存失败不会再次发送邮件；
- 发送成功后把最后联系时间和邮件同步状态写回飞书，失败时后台重试；
...
- 管理页提供上下中英对照的可编辑回复草稿；可根据中文一键重新生成英文，也可手工编辑并保存两种语言；
...
自动任务仍然只读取邮箱、分析和写回飞书；邮件发送必须由管理员逐封点击`确认并发送（不可撤回）`，没有自动发送功能。
...
| PUT | `/api/admin/mailboxes/:mailboxId/messages/:messageId/draft` | ADMIN_TOKEN | 翻译或保存可编辑的中英文回复草稿 |
| POST | `/api/admin/mailboxes/:mailboxId/messages/:messageId/send` | ADMIN_TOKEN | 明确人工确认后向原始发件人回复一次 |
...
AI分析完成后，工作台上方显示可编辑中文草稿，下方显示可编辑英文草稿。修改中文后点击`根据中文生成英文并保存`，程序只调用一次翻译接口；如果手工修改了英文，点击`保存当前中英文`即可加密保存。保存后的英文会写回飞书`AI回复草稿`，但仍需人工点击发送。
...
如果AI已经分析成功、但飞书某个字段不存在或单选值不可用，工作台仍会显示结果，并显示`等待写回/写回失败`；修好字段后点击`重新分析`即可再次写回。所有英文草稿都必须人工审核。

## 第五阶段人工确认发送

1. 在试点邮箱卡片点击`配置SMTP`；
2. 填写SMTP服务器、端口、TLS方式、用户名、应用专用密码、Sent文件夹和签名；
3. 点击`测试SMTP`。测试只验证连接和登录，不发送邮件；
4. 测试成功后点击`启用SMTP发送`。本阶段同一时间只能启用一个邮箱；
5. 打开一封已经完成AI分析的达人回复，检查并保存中英文草稿；
6. 核对页面显示的最终收件人，点击`确认并发送（不可撤回）`；
7. 浏览器再次显示发件邮箱、收件人和主题，只有点击确定后才调用SMTP；
8. 发送成功后页面显示发送时间、Sent副本状态和飞书发送状态。

系统只回复原邮件的第一个有效外部发件地址，不允许页面临时改成其他收件人；`In-Reply-To`和`References`来自原邮件。发送状态先在PostgreSQL中原子锁定，重复点击会被拒绝。SMTP成功但Sent副本保存失败时，状态仍为`已发送`，绝不会为了补Sent而再次发送。
...
- [ ] 重复同步不会重复保存同一封邮件；
- [ ] `/health`显示版本`5.0.0`且`openaiConfigured: true`；
- [ ] 管理页顶部显示过去24小时摘要；
- [ ] 打开最近邮件后可看到原邮件与AI分析双栏；
- [ ] 一封达人回复能生成中文摘要、报价提取和英文草稿；
- [ ] 匹配成功的邮件能把AI字段写回飞书；
- [ ] 页面和日志中不显示`OPENAI_API_KEY`；
- [ ] 一个试点邮箱可以加密保存SMTP配置，接口不会返回密码；
- [ ] `测试SMTP`成功且测试过程中没有发送邮件；
- [ ] 只有一个邮箱能够启用SMTP发送；
- [ ] 未点击最终确认时不会发送任何邮件；
- [ ] 一封测试回复能进入原邮件线程，发件邮箱与原收件邮箱一致；
- [ ] 同一封邮件重复点击会被服务端阻止；
- [ ] Sent副本状态、发送时间和飞书写回状态在页面可见；
