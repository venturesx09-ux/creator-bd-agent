...
- 匹配达人后，主表只写回当前保留字段中的`最近发件邮箱`、`AI中文摘要`、`报价金额`、`报价币种`、`项目`、`合作阶段`、`交付内容`、`邮件同步状态`、`邮件分类`、`回复邮件明细`和`累计回复邮件数`；不会改写`达人id`、`主页链接`或`达人邮箱`；
...
- 匹配成功后写回最近发件邮箱、项目、邮件同步状态和邮件分类；普通达人回复仅在早期阶段推进为已回复。
...
每次同步后，程序会依次尝试：飞书字段中的完整发件邮箱、当前回复签名中的Instagram/TikTok/YouTube/X个人主页、发件邮箱用户名、邮件标题中的`@达人ID`、回复线程中的`Hi/Hello/Hey/Dear + 达人ID`。签名匹配只读取当前回复，不读取引用历史中的品牌或其他社媒链接。飞书`达人id`和`主页链接`都可建立达人ID索引，支持纯ID、`@ID`、Instagram/TikTok/YouTube/X链接，以及`IG: ID`等带平台前缀格式。所有方式都做不区分大小写的精确匹配，只有唯一命中才写回，不使用可能误匹配达人的模糊猜测。首次未命中时会自动强制刷新飞书索引再尝试一次，处理新添加达人或缓存尚未更新的情况。邮箱和达人ID索引都只以不可逆SHA-256哈希保存在PostgreSQL，重启后直接复用，并每30分钟后台刷新，也可以在管理页手动立即刷新。升级前同步的旧邮件会自动重新匹配；待匹配查询记录每封邮件上次尝试时间，按“最久未尝试”轮转，不会再因固定取最新500封而让昨天的邮件长期排不到。未匹配邮件会在后续同步时重试，并在配置`FEISHU_UNMATCHED_TABLE_ID`后写入独立子表。程序在PostgreSQL保存对应子表记录ID，因此重复同步只更新原行；邮件以后匹配成功时，原行自动更新处理状态、最终记录ID和解决时间。匹配成功后只在主表保留字段中写回，并只把早期合作阶段推进为“已回复”。同时，每封匹配成功的达人回复会以邮件唯一ID写入内部事件表并重新计算同一达人行的累计回复数和逐封明细，重复同步不会重复计数。`回复邮件明细`使用邮件实际收件时间（北京时间），不使用批量写回时间。如果引用历史被对方完全删除，当前仅同步INBOX的版本无法还原首封发件内容。

飞书达人主表只保留以下字段，名称和类型必须完全一致：
...
`累计回复邮件数`只统计被分类为`达人回复`且成功匹配到该达人行的唯一邮件。邮箱INBOX总数还可能包含自动回复、退信、群发通知及未匹配邮件，因此两者不是同一统计口径。升级后，历史已匹配回复会按每个邮箱每批最多500封自动补写累计数量和逐封明细，无需手动点击分析。

## 第四阶段AI分析

新同步和升级前已存在的`达人回复`都会在后台进入AI分析并逐批回填，保证匹配成功的已回复记录拥有中文摘要和报价信息。升级到5.1后，旧报价结构的邮件会按每个邮箱每批最多100封从IMAP重新读取原始邮件并重新分析一次，完成后不会重复消耗额度。HTML邮件会先转换为纯文本，最多保留30,000字符用于覆盖较长的历史线程。报价分析同时读取最新回复和引用历史，区分单价、总价、区间和多个套餐，并避免把品牌预算、产品价值、联盟佣金或已完成项目的invoice误认成达人当前报价。

AI失败重试采用有限退避：只有超时、429限流/余额不足和服务端异常会自动重试，分别在30分钟、2小时、12小时后最多重试3次；第4次失败后停止自动调用，避免持续消耗API额度。401/403密钥错误、结构化输出失败等永久性错误不会自动重试。升级到5.5.1时，旧版本遗留的可重试失败邮件只会被恢复到这套有限重试队列，不会无限循环。

5.5.2修复了失败状态写入PostgreSQL时参数类型推断冲突的问题。如果失败状态暂时无法写入数据库，进程会对该邮件启用30分钟内存冷却，并单独记录原始AI错误码，避免邮件仍处于待处理状态时连续调用API。

5.5.3在管理页增加`测试AI连接`和`重试失败AI分析`。连接测试只发起一次小型结构化分析请求；测试成功后10分钟内可手动恢复最多500封`OPENAI_AUTH_FAILED`邮件。每次恢复后授权立即失效，必须重新测试才能恢复下一批，避免错误配置下批量反复调用。401与AIHubMix常见403原因也会分别显示为认证、余额、IP限制、模型权限或账号状态错误。

AI回复草稿功能当前已停用：模型固定返回空草稿，工作台不显示草稿编辑区，飞书不再写入`AI回复草稿`，草稿和发送API也不再暴露。后续回复模板和界面将独立重新设计。

AI结果只写入上述主表白名单中的以下字段：
...
没有识别到报价时，`报价金额`和`报价币种`会清空，中文摘要会明确说明未提及报价。报价原文、标准化报价、区间和多个套餐仍保存在内部加密分析结果中，但主表不再写入已删除的`报价`字段。
